...
class ItsRemoteAppStack(TerraformStack):
    def __init__(self, scope: Construct, id: str, tfstate_key: str, customer: str):
        ...
        its_remoteapp_rg = resource_group.ResourceGroup(
            self,
            "its-remoteapp-stack",
            name="its-remoteapp-stack",
            location="westus2",
        )

        # Create AVD workspace
        its_remoteapp_workspace = virtual_desktop_workspace.VirtualDesktopWorkspace(
            self,
            "its-remoteapp-workspace",
            name="its-remoteapp-workspace",
            resource_group_name=its_remoteapp_rg.name,
            location="westus2",
        )

        # Create AVD host pool
        its_remoteapp_host_pool = virtual_desktop_host_pool.VirtualDesktopHostPool(
            self,
            "its-remoteapp-host-pool",
            name="its-remoteapp-host-pool",
            location="westus2",
            resource_group_name=its_remoteapp_rg.name,
            type="Pooled",
        ...
        its_remoteapp_host_pool_registration_info = virtual_desktop_host_pool_registration_info.VirtualDesktopHostPoolRegistrationInfo(
            self,
            "its-avd-host-pool-registration-info",
            hostpool_id=its_remoteapp_host_pool.id,
            expiration_date=rfc3339_expiration,
        )

        # Create AVD app group
        its_remoteapp_app_group = (
            virtual_desktop_application_group.VirtualDesktopApplicationGroup(
                ...
                resource_group_name=its_remoteapp_rg.name,
                type="RemoteApp",
                host_pool_id=its_remoteapp_host_pool.id,
                friendly_name="its-avd-app-group",
                default_desktop_display_name="its-avd-desktop",
            )
        )

        its_remoteapp_application = virtual_desktop_application.VirtualDesktopApplication(
            self,
            "its-remoteapp-application",
            name="its-remoteapp-application",
            application_group_id=its_remoteapp_app_group.id,
            friendly_name="Quickbooks",
            path="C:\\Program Files\\Intuit\\QuickBooks Enterprise Solutions 22.0\\QBWEnterpriseContractor.exe",
            command_line_argument_policy="DoNotAllow",
            show_in_portal=False,
            icon_path="C:\\Program Files\\Intuit\\QuickBooks Enterprise Solutions 22.0\\QBWEnterpriseContractor.exe",
            icon_index=0,
        )

        # Create AVD app group association with workspace
        its_remoteapp_group_association = virtual_desktop_workspace_application_group_association.VirtualDesktopWorkspaceApplicationGroupAssociation(
            self,
            "its-avd-app-group-association",
            workspace_id=its_remoteapp_workspace.id,
            application_group_id=its_remoteapp_app_group.id,
        )
